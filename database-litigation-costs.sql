-- =====================================================
-- LITIGATION COSTING MODULE - DATABASE SCHEMA
-- Department of Lands & Physical Planning (DLPP)
-- Papua New Guinea
-- =====================================================
-- This module captures all legal and financial costs
-- incurred by DLPP and the State on a per-case basis
-- =====================================================
...
CREATE TABLE public.cost_categories (
    ...
    description TEXT,
    category_group VARCHAR(100), -- e.g., 'Legal Fees', 'Court Fees', 'Settlements', 'Penalties'
...
CREATE TABLE public.litigation_costs (
    ...
    cost_type VARCHAR(100) NOT NULL, -- Denormalized for quick access
    amount DECIMAL(15, 2) NOT NULL CHECK (amount >= 0),
    currency VARCHAR(3) DEFAULT 'PGK', -- Papua New Guinea Kina
    ...
    -- Responsibility
    responsible_unit VARCHAR(255), -- e.g., 'Legal Division', 'Finance', 'Secretary's Office'
    responsible_authority VARCHAR(255), -- e.g., 'Solicitor General', 'State', 'DLPP'
    ...
    payee_type VARCHAR(50), -- 'law_firm', 'court', 'individual', 'company', 'government'
...
CREATE TABLE public.litigation_cost_documents (
    ...
    document_type VARCHAR(100), -- 'invoice', 'receipt', 'court_order', 'payment_voucher', 'other'
...
-- Cost Categories: Only admins can modify
...
-- Litigation Costs: Insert/Update for legal and finance roles
CREATE POLICY "Legal and Finance can manage litigation costs"
    ...
    WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.profiles
            WHERE profiles.id = auth.uid()
            AND profiles.role IN ('admin', 'legal_officer', 'registrar', 'director')
        )
    );

CREATE POLICY "Legal and Finance can update litigation costs"
...
-- the Litigation Costing Module tables and functions.
-- =====================================================

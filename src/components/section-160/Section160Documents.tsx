...
import {
  SECTION_160_DOCUMENT_TYPES,
  getSection160DocTypeLabel,
...
interface Section160DocumentsProps {
  applicationId: string;
  caseId: string | null;
  canUpload?: boolean;
}

export function Section160Documents({
  applicationId,
  caseId,
  canUpload = true,
}: Section160DocumentsProps) {
  const [docs, setDocs] = useState<AppDoc[]>([]);
  const [loading, setLoading] = useState(true);
  const [uploading, setUploading] = useState(false);
  const [docType, setDocType] = useState(SECTION_160_DOCUMENT_TYPES[0].value);
  ...
  const loadDocs = useCallback(async () => {
    try {
      const { data, error } = await (supabase as any)
        ...
        .eq('section_160_application_id', applicationId)
        .order('uploaded_at', { ascending: false });
      if (error) throw error;
      setDocs((data as AppDoc[]) || []);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `${caseId ?? 'section-160'}/${fileName}`;
      ...
      const { error: insErr } = await (supabase as any).from('documents').insert({
        case_id: caseId,
        section_160_application_id: applicationId,
        title: file.name,
        file_url: publicUrl,
      ...
      await logAudit('create', 'section_160', applicationId, 'section_160_document', {
        document_type: docType,
        title: file.name,
      });

      toast.success(
        caseId ? 'Document uploaded and linked to the case' : 'Document uploaded to the application',
      );
      setFile(null);
      loadDocs();
    } catch (err) {
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          {!caseId && (
            <div className="mb-3 flex items-start gap-2 rounded-md border border-sky-200 bg-sky-50 p-2.5 text-xs text-sky-800">
              ...
              This application is not linked to a case yet. Documents are still stored on the
              application and will also appear in the case document list once linked.
            </div>
          )}
          <div className="grid gap-3 sm:grid-cols-[240px_1fr_auto] sm:items-end">
            <div className="space-y-1.5">
              ...
              <Select value={docType} onValueChange={setDocType}>
                ...
                <SelectContent>
                  {SECTION_160_DOCUMENT_TYPES.map((t) => (
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {docs.map((d) => (
              <div key={d.id} className="flex items-center gap-3 rounded-lg border border-slate-200 p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="text-[11px]">
                      {getSection160DocTypeLabel(d.document_type)}
                    </Badge>

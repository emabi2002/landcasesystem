...
export function AddCostDialog({ caseId, onCostAdded, trigger }: AddCostDialogProps) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [categories, setCategories] = useState<CostCategory[]>([]);
  const [error, setError] = useState<string | null>(null);

  const [formData, setFormData] = useState<CreateLitigationCostInput>({
    case_id: caseId,
    cost_type: '',
    amount: 0,
    currency: 'PGK',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to add cost entry');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger || (
          <Button className="bg-[#8B2332] hover:bg-[#6B1A26]">
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFormData(prev => ({
                  ...prev,
                  payment_status: value as CreateLitigationCostInput['payment_status'],
            ...
            <div className="space-y-2">
              <Label htmlFor="amount_paid">Amount Paid (PGK)</Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">K</span>
                <Input
                  id="amount_paid"

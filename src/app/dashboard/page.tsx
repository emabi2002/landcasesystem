...
export default function EnhancedDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const casesData = (cases as { status: string; case_type: string; region?: string; created_at: string; closure_date?: string }[]) || [];

      // Calculate statistics
      const statistics: CaseStats = {
        total: casesData?.length || 0,
        open: casesData?.filter(c => !['closed', 'settled'].includes(c.status)).length || 0,
        closed: casesData?.filter(c => ['closed', 'settled'].includes(c.status)).length || 0,
        thisMonth: casesData?.filter(c => new Date(c.created_at) >= thisMonthStart).length || 0,
        thisYear: casesData?.filter(c => new Date(c.created_at) >= thisYearStart).length || 0,
        lastYear: casesData?.filter(c =>
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const opened = casesData?.filter(c => {
          const createdAt = new Date(c.created_at);
          return createdAt >= monthStart && createdAt <= monthEnd;
        }).length || 0;

        const closed = casesData?.filter(c => {

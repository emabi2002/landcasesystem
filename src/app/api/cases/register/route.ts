...
export async function POST(request: NextRequest) {
  try {
    ...
    // STEP 9: EXECUTIVE WORKFLOW - Automatic Notifications
    console.log('🔔 Initiating Executive Legal Oversight Workflow...');

    // Determine if this is a new or existing case
    const isNewCase = !formData.court_file_number || formData.status === 'under_review';

    // Build case summary for notifications
    const caseSummary = [
      `Case Number: ${caseNumber}`,
      `Title: ${caseTitle}`,
      `Court Reference: ${formData.court_file_number || 'Not provided'}`,
      `Matter Type: ${formData.matter_type || 'Not specified'}`,
      `DLPP Role: ${formData.dlpp_role === 'plaintiff' ? 'Plaintiff' : 'Defendant'}`,
      `Priority: ${formData.priority || 'medium'}`,
      `Status: ${isNewCase ? 'NEW CASE' : 'EXISTING CASE'}`,
    ].join('\n');

    try {
      // Call database function to notify executive officers
      const { data: notificationResult, error: notifyError } = await supabase
        .rpc('notify_executive_officers', {
          p_case_id: caseId,
          p_case_number: caseNumber,
          p_case_title: caseTitle,
          p_court_reference: formData.court_file_number || null,
          p_is_new_case: isNewCase,
          p_case_summary: caseSummary,
          p_created_by: formData.user_id
        });

      if (notifyError) {
        console.error('⚠️ Warning: Could not send executive notifications:', notifyError);
      } else {
        console.log(`✅ Executive notifications sent to ${notificationResult || 0} officers`);
      }

      // Initialize executive workflow tracking
      const { error: workflowError } = await supabase
        .rpc('initialize_executive_workflow', {
          p_case_id: caseId,
          p_is_new_case: isNewCase,
          p_case_summary: caseSummary
        });
      ...
      if (workflowError) {
        console.error('⚠️ Warning: Could not initialize workflow tracking:', workflowError);
      } else {
        console.log('✅ Executive workflow tracking initialized');
      }

    } catch (execError) {
      console.error('⚠️ Executive workflow error (non-fatal):', execError);
      // Don't fail the entire registration if notifications fail
    }

    console.log('🎉 Case registration complete with all normalized data and executive oversight!');

    return NextResponse.json({
      success: true,
      case: newCase,
      message: 'Case registered successfully with all related data. Executive officers have been notified.',
      executiveWorkflowInitiated: true
    });

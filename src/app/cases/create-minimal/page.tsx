...
import { ArrowLeft, FileText, AlertCircle, CheckCircle2, Upload } from 'lucide-react';
...
export default function CreateMinimalCasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate that at least one originating document is uploaded
    if (uploadedDocuments.length === 0) {
      toast.error('Please upload at least one originating document before creating the case');
      return;
    }
    ...
    try {
      ...
      // Link uploaded documents to the case
      if (uploadedDocuments.length > 0) {
      ...
      toast.success(
        `Case ${caseNumber} created successfully with ${uploadedDocuments.length} document(s)!`,
        { duration: 3000 }
      );
          ...
          <Card className="border-2 border-slate-200">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500">
                  <strong>Required:</strong> Upload the originating document(s) for this case (e.g., court summons, notice, complaint).
                  All documents are stored securely and linked to this case.
                </p>
              </div>
